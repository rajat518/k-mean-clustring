		...
		public void configure(JobConf job) {
			try {
				...
				if (cacheFiles != null && cacheFiles.length > 0) {
					...
					try {
						// Read the file split by the splitter and store it in the list
...
finally 
{
		...
		/*
		  The function will find the minimum center of the point and gives result  it to  the reducer
		 */
		@Override
		public void map(LongWritable key, Text value,
			...
			double m1, m2 = Double.MAX_VALUE, nearest_center = mCenters
					.get(0);
	...
	public static class Reduce extends MapReduceBase implements
		...
		/*
		  Reduce function will give all the points to that center and calculate
		  the next center for the points
		 */
		@Override
	...
	public static void run(String[] args) throws Exception {
		...
		while (isdone == false) {
			...
			if (it == 0) {
				...
				// upload the file to hdfs and rewrite the if file already exists.				DistributedCache.addCacheFile(hdfsPath.toUri(), conf);
			} else {
				Path hdfsPath = new Path(again_input + OUTPUT_FIE_NAME);
				// upload the file to hdfs and rewrite the if file already exists.
			...
			if (it == 0)
 {
			...
			// Sorting the old and new centroids and Checking the condition for convergence
